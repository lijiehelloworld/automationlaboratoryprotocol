...
一个 Device Node 最多公开一个活动的逻辑设备智能体。供应商内部可以组合多个模型或模块，但对外仍表现为一个 `device_agent`。
...
- `modes` MUST 至少包含 `plan_only`。
- `execute_workflow` 和 `execute_experiment` MAY 不提供；声明后必须遵守完整执行规范。
- 声明 `execute_experiment` 的设备 MUST 同时实现 `ALP-Operations`，使完整实验可以显式查询、取消或处理输入等待。
- 智能体权限不能超过调用者权限与智能体范围的交集。
- 凭据、模型密钥和供应商内部端点 MUST NOT 出现在 Manifest 中。
...
返回智能体定义、支持模式、输入 Schema 和当前可用状态。该方法 MUST NOT 移动设备。
...
执行模式 MUST 引用已生成的 `proposal_id`、Manifest 摘要、Workflow 摘要、最新 revision 和可信授权。Server MUST 重新执行 Workflow 校验，然后通过 `workflows/run` 或普通 `invoke` 执行。

##### Natural-language Experiment Execution

声明 `execute_experiment` 的单设备智能体可以接收完整自然语言实验目标，并在一个受控 Operation 中完成：

1. 解析目标、对象、容器、耗材、参数和结果要求；
2. 读取当前设备、对象、耗材和环境状态；
3. 选择已有 Workflow，或在授权范围内生成临时计划；
4. 需要时控制机器人完成本设备边界的耗材或实验对象进出；
5. 调用已声明的 Action、Workflow 和受限脚本步骤；
6. 处理中间 `input_required` 和人工授权；
7. 汇总步骤结果、Evidence、异常和最终状态；
8. 对可复用计划创建或修改 Workflow 草稿并提交人工审批。
...
完整实验通常返回 `resultType=operation`。Operation MUST 显式记录当前阶段：
...
智能体可以生成临时计划，但每个物理步骤仍必须映射到已声明的 Action、Workflow、Transfer 或 Script。自然语言本身不能成为原始设备命令。

如果没有合适的 Workflow，智能体可以：

- 在当前授权范围内直接编排已声明 Action 完成本次实验；
- 创建 `draft` Workflow；
...
3. 使用调用者权限和智能体权限的交集；
4. 不跳过人工授权；
5. 不修改 Manifest、Constraint、锁定 Workflow 或审计记录；
6. 只选择已声明的 Action、Transfer、Workflow 和受限 Script；
7. 在状态 revision 改变后使旧 Proposal 失效；
8. 遇到 `unknown` 时停止自动执行并请求回读或人工处理；
9. 不承担硬实时闭环、急停、安全门或运动限位；
10. 在远程服务不可用时不影响普通设备控制接口。
11. 创建和修改 Workflow 时只能操作未锁定草稿，并保留作者、原因和 revision。
12. 修改 Device Supply 状态时必须使用 Physical Resource 接口，不能直接改写设备内部数据库。

调用方始终通过 Server 的 `agent/*` 方法访问设备智能体。智能体内部部署方式不得改变 ALP 对外数据结构。
