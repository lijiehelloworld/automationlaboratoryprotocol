...
FSP 的对象维度说明某项操作能够识别或处理的实验器皿，以及调用该操作时器皿必须满足的状态。目录只收录具有明确器皿或样品盛放语义的对象。
...
容器定义作为 FSP 设备定义的 `object` 维度提供。多项操作可以使用相同容器类型，但每项操作仍须明确声明自身允许的容器变体和状态。
...
```json
{
  ...
  "params": {
    ...
    "operation_id": "read_container_state",
    "arguments": {
      "container_ids": [1, 2]
    }
  }
...
`fsp/read` 只是读取类请求入口；`read_container_state` 是 FSP 设备定义中声明的具体读取操作。其参数和返回字段必须符合该操作的输入与输出 Schema。
...
- 设备只能返回 FSP 设备定义中已经声明的状态字段；
- 状态读取必须包含观测时间或修订号；
- 操作产生的状态变化必须与操作定义声明的 `effects` 一致；
- 强制条件失败时不得提交预期新状态；
...
FSP 负责设备可见的容器和状态；ALL 语言引擎使用同一结构生成、仿真和执行中间程序。
...
FSP、对象状态和 ALL 中间程序之间通过以下公共字段传递状态：
...
编译、仿真和执行必须沿用这些字段，不替换成没有实现依据的新标识体系。
...
编译/解释器获得的上下文至少包含科学目标、对象身份、容器类型和编号、当前容器与样品状态、已有样品体积、FSP 设备定义、强制约束以及已有证据和来源。
