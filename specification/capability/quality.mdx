...
输入是按照[技能提取](/specification/capability/overview)生成的设备技能描述。
...
操作: 加液_物料绑定
初始状态: container_status=无盖
参数: container_ids、dispensing_plan
约束: 单次移液量不超过1mL；单种溶液总加注量不超过3mL
状态更新: sample_status 空→液体
返回值: containers[]
```
...
1. 检查元数据、操作名称和来源修订号是否存在；
2. 对每个参数检查名称、类型、必填性、单位、范围、枚举和嵌套字段是否完整且可解析；
3. 对每条前置条件、约束、状态变化和返回值建立对应关系，确认操作调用前后状态能够闭合；
...
| 字段 | 类型 | 作用 |
| --- | --- | --- |
| `station_name` | `string` | 被评估能力的稳定名称 |
