---
title: "绑定"
description: "把验证后的函数调用绑定到可用工作站、操作和参数"
---

## 模块作用

技能匹配读取合格设备技能库，根据方法需要的函数、容器状态、参数范围和设备可用性选择工作站。ALL 不定义没有验证依据的通用匹配分数；绑定结果只表达已匹配步骤和无法绑定步骤。
...
| 输入字段 | 作用 |
| --- | --- |
| `method_call.namespace` | 指定函数所属工作站能力空间 |
...
无法识别或无法绑定的步骤写入 `unknown_steps`，并保留原始步骤、失败原因和缺少的能力。存在未知步骤时，执行计划不得进入物理执行。
