...
├── style.css             # 网站全局样式
├── AGENTS.md             # 协作约定与研究结论，仅供仓库维护
└── README.md             # 项目入口与使用说明
```

## Git 管理与网站发布范围

| 内容 | 提交到 GitHub | 发布到 Mintlify |
| --- | --- | --- |
| `docs/`、`images/`、`index.mdx`、`style.css` | 是 | 是 |
| `docs.json` | 是 | 作为网站配置使用 |
| `README.md`、`AGENTS.md`、`planning/` | 是 | 否 |
| `.gitignore`、`.mintignore` | 是 | 作为管理规则使用 |
| `references/`、`artifacts/` | 否，仅本地保存 | 否 |
| 依赖、缓存、编辑器配置 | 否 | 否 |

`.gitignore` 控制 Git 收录范围；`.mintignore` 控制网站处理和发布范围。GitHub 中的维护记录不等于网站页面；公开 GitHub 仓库中的 `AGENTS.md` 和计划仍可被公开阅读，不能放入秘密凭据或私密材料。
...
目录迁移提交需要同时包含旧文件删除、新文件加入和导航更新。提交前查看暂存区差异，确保没有研究原件、缓存或临时文件；推送到已绑定的生产分支可能触发网站发布。
