---
title: "设备仪器接口要求与映射示例"
description: "说明设备仪器接口必须包含的三个维度，以及转发器如何完成映射。"
---
...
流程伪代码见[转发器](/docs/specification/fsp/adapter)：用同一个加液例子说明动作怎么定义、执行前怎么检查、设备命令怎么转换。完整实现见 [GitCode 示例源码](https://gitcode.com/mimedal/all/blob/main/examples/fsp_three_parts.py)。
