---
title: "厂商设备接口要求与映射示例"
description: "说明厂商设备接口必须包含的三个维度，以及适配器如何完成映射。"
---
...
厂商可以继续使用原有的 HTTP API、CLI、MCP 工具、SDK、RPC 或驱动，也可以把三方面信息分布在操作调用、状态查询、结果回调或事件中。FSP 服务中的适配器按预先写好的规则完成字段、单位和结果映射；如果厂商接口没有提供某项实际信息，适配器只能把它标为未知，不能临时猜测或用请求值代替。
