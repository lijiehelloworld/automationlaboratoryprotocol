---
title: "请求与响应格式"
description: "说明设备仪器接口应如何表达输入、结果、状态版本和错误。"
---

## 适用范围

本规范定义设备仪器接口必须包含的数据结构与行为。每个厂商接口都要覆盖操作、对象和系统三个维度的内容，但不要求拆成三套独立接口，也不规定 HTTP 路径、CLI 命令、MCP 工具名、RPC 名称或消息封装格式。实现可以自行选择连接和传输方式。

## 通用操作输入

语言引擎先通过 FSP 读取设备能力、使用边界和约束，用于生成流程与仿真；执行器在下发前再次查询当前设备和对象状态。下面的操作输入用于这些检查通过后的任务执行。能力查询和状态查询的字段要求见[能力门控](/docs/specification/fsp/capability-gating)和[系统](/docs/specification/device-integration/system)，完整工作流程见[协议概述](/docs/specification/overview)。
