...
执行器接收已通过仿真的任务段。以顺序执行计划为例，一份计划至少包含一个稳定的幂等键、按顺序排列的步骤，以及设备和对象的预期版本。
...
![执行与证据核对](/images/all-executor-flow-gptimage.png)
...
步骤按实验流程的依赖关系排列。设备切换时，应确认样品转移、装载和现场准备条件，并记录完成证据。例如：
...
需要多轮返回同一设备的工艺，例如“离心—清洗—再次离心”，应保留原有工艺顺序。每次再次使用设备前，重新确认设备和样品状态；按任务段执行时，各段应保留对象引用和前后依赖。
...
| 场景 | 处理结果 |
| --- | --- |
| 任务通过仿真，当前状态未变化 | 查询能力与状态，校验后提交并跟踪同一任务 |
| 设备被占用，或能力与状态版本发生变化 | 不下发动作，返回原因并重新确认流程 |
| 计划受理后设备状态变化 | FSP 在执行动作前再次检查，条件不满足时停止 |
| 使用原请求键重复提交 | 返回原任务，设备命令次数不增加 |
| 动作回执丢失 | 结果标为未知，保留待核实状态，停止依赖步骤 |
| 达到本次查询次数上限 | 返回当前任务状态，后续使用原任务继续查询 |

加液案例的仿真预测量为 0.5 mL，模拟设备报告量为 0.498 mL。执行器使用设备报告的数值和停止证据判断结果，不用预测值替代实际结果。

对应实现见 [任务执行与 FSP 模拟服务](https://gitcode.com/mimedal/all/blob/main/examples/language_engine/executor.py)。[验证用例](https://gitcode.com/mimedal/all/blob/main/tests/test_language_engine.py)检查下发前状态变化、受理后状态变化、幂等冲突、任务续查及结果未知。运行方式见[示例说明](https://gitcode.com/mimedal/all/blob/main/examples/language_engine/README.md)。
